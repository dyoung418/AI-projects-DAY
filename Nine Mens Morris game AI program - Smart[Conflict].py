...
def multiplayer_search(game, state, t=None, d=None, cutoff_test=None, eval_fn=None):
    ...
    def max_player_value(state, depth):
        ...
        if cutoff_test(state, depth):
            ...
            for a in game.actions(state):
                next_node_value = [max_player_value(game.result(state, a), depth+1)[0], a]
    ...
    max_util_and_action = max_player_value(state, 0)
    return max_util_and_action[1] #return the associated action with the max value utility


def alphabeta_multiplayer(game, state, t=None, d=None, cutoff_test=None, eval_fn=None):
    # This is now no different than multiplayer_search() since I
    # figured out that alphabeta pruning won't work for multiplayer.
    '''Given a state in a multi-player game, calculate the best move by
    ...
    def max_player_value(state, alpha_values, depth):
        '''Return the max utility value for the player whose move it is during
        "state".'''
        player = game.to_move(state)
        if cutoff_test(state, depth):
        ...
        v = [(-infinity,)*game.num_players(), None] 
        for a in game.actions(state):
            next_node_value = [max_player_value(game.result(state, a), alpha_values, depth+1)[0], a]
            if next_node_value[0][player] > v[0][player]:
                v = next_node_value # v is the best utility so far (for current player)
            ...
            #        DECREASE IN OTHER PLAYER'S UTILITY.
            # Use with Caution!
    ...
    # Body of alphabeta_search starts here:
    max_util_and_action = max_player_value(state, [-infinity,]*game.num_players(), 0)
    return max_util_and_action[1] #return the associated action with the max value utility
...
class NineMensMorris(Game):
    ...
    def utility(self, state, player):
        ...
        (_, board, bstones, unplaced, removeok, resign) = state
        me = player
        ...
        stone_count = {me:bstones[me]+unplaced[me], opponent:bstones[opponent]+unplaced[opponent]- 1 if removeok else 0}
        ...
        # Need to create a "2 single rows connected by a blank" category (i.e. with one stone it becomes two doubles.)

        
        # util structure: a 'min' score or lower will get 0 utility and a 'max' or higher score will get full 'weight'
        # scores between 'min' and 'max' will be trended in one of three ways:
        #    * 'linear' -- straight line from 0 to 'weight' as measure moves from 'min' to 'max'
        #    * 'sq' -- x squared curve from 0 to 'weight' as measure moves from 'min' to 'max' (slow to ramp near 'min', but strong acceleration at 'max')
        #    * 'sqrt' -- x square-root curve from 0 to 'weight' as measure moves from 'min' to 'max' (quick to ramp near 'min', but slows down at 'max')
        util = { 'liberties':       {'min':0, 'max':4, 'weight':.35, 'curve':'sq'}, #don't care above 4 but want to heavily weight 0 and 1 liberty
                                                                      #setting max at 4 intrinsically gives preference to locations with
                                                                      #4 neighbors as opposed to locations with 2 or 3.
                 'stone count':     {'min':0, 'max':9,  'weight':.1, 'curve':'sq'},
                 'doubles':         {'min':0 , 'max':2, 'weight':.1, 'curve':'linear'},
                 'milles':          {'min':0 , 'max':1, 'weight':.3, 'curve':'linear'},
                 'alt milles':      {'min':0 , 'max':1, 'weight':.15, 'curve':'linear'} }
        score = 0
    ...
    def normalized_utility(self, metric, u):
        ...
        'u' is a dictionary with the following keys:
            ...
            '''
        exponent = {'linear':1, 'sq':2, 'sqrt':0.5}[u['curve']]
        if metric <= u['min']: return 0
        elif metric >= u['max']: return u['weight']
        else:
            return (u['weight'] * ((float(metric) - u['min'])/(u['max']-u['min'])))**exponent
